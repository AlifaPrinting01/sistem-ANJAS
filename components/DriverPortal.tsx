...
const DriverPortal: React.FC<DriverPortalProps> = ({ routes, setRoutes, onTriggerAlert }) => {
  ...
  const deleteStudent = (studentId: string, name: string) => {
    if (confirm(`Apakah Anda yakin ingin menghapus data siswa "${name}"? Tindakan ini permanen dan akun Orang Tua tidak akan bisa login lagi.`)) {
  ...
  const finishRoute = () => {
    if (confirm(`Selesaikan penjemputan Kloter ${activeBatch}? Semua siswa yang sudah naik akan ditandai sampai sekolah.`)) {
      const updatedStudents = myRoute.students.map(s => {
  ...
  const totalCollected = myRoute.students ? myRoute.students.reduce((acc, s) => {
    return s.isPaid ? acc + FEES[s.feeType] : acc;
  }, 0) : 0;

  const notificationStudents = myRoute.students ? myRoute.students.filter(s => 
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-red-50 border-2 border-red-100 p-4 rounded-3xl flex items-center justify-between gap-4">
        ...
        <button onClick={() => setShowEmergencyPanel(true)} className="bg-red-600 text-white px-4 py-2 rounded-xl text-xs font-black uppercase shadow-lg shadow-red-200 hover:bg-red-700 transition-all active:scale-95 flex items-center gap-2"><Megaphone size={14} /> LAPOR KENDALA</button>
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[110] flex items-center justify-center p-4">
           <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl p-8 animate-in zoom-in duration-300">
              <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-3"><div className="p-3 bg-red-100 text-red-600 rounded-2xl"><AlertTriangle size={24} /></div><h3 className="text-xl font-black text-slate-900">Broadcast Darurat</h3></div>
                <button onClick={() => setShowEmergencyPanel(false)} className="p-2 hover:bg-slate-100 rounded-full text-slate-400 transition-colors"><X size={20} /></button>
              </div>
              ...
              <div className="space-y-3">
                {EMERGENCY_REASONS.map(reason => (
                  <button key={reason.id} onClick={() => sendEmergencyAlert(reason.msg)} className="w-full text-left p-4 border-2 border-slate-100 rounded-2xl hover:border-red-200 hover:bg-red-50 transition-all group">
            ...
            {notificationStudents.map(s => (
              <div key={s.id} className={`flex items-center gap-4 py-3 px-4 rounded-2xl border-2 transition-all ${s.status === StudentStatus.WAITING ? 'bg-emerald-50 border-emerald-200 shadow-md scale-[1.02]' : 'bg-slate-50 border-slate-100'}`}>
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2"><p className="text-sm font-bold text-slate-900">{s.name}</p><span className="text-[9px] font-black bg-slate-200 text-slate-600 px-1.5 py-0.5 rounded uppercase">Kloter {s.batch}</span></div>
                  <p className={`text-[10px] font-black uppercase tracking-tighter ${s.status === StudentStatus.WAITING ? 'text-emerald-600' : 'text-slate-500'}`}>
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-6">
          <div className="flex items-center justify-between mb-6 px-2">
            ...
            <button onClick={() => setIsAddingStudent(!isAddingStudent)} className="text-[10px] font-black bg-slate-900 text-white px-3 py-1.5 rounded-xl uppercase flex items-center gap-1.5 hover:bg-slate-800 transition-colors"><Plus size={14} /> Tambah Siswa</button>
          </div>
          ...
          {isAddingStudent && (
            <div className="mb-6 p-6 bg-slate-50 rounded-2xl border border-slate-200 animate-in slide-in-from-top duration-300">
               ...
               <form onSubmit={(e) => {
                 e.preventDefault();
                 const studentToAdd: Student = { id: (Date.now() % 10000).toString(), name: newStudent.name, className: newStudent.className, batch: newStudent.batch, address: newStudent.address, parentName: newStudent.parentName, status: StudentStatus.AT_HOME, isPaid: false, feeType: 'NORMAL' };
                 const updatedStudents = [...myRoute.students, studentToAdd];
          ...
          <div className="space-y-4">
            {filteredStudents.length === 0 ? (
              <div className="p-16 text-center border-2 border-dashed border-slate-100 rounded-3xl flex flex-col items-center gap-3">
                ...
                <div><p className="text-slate-400 text-sm font-bold">Belum ada siswa di Kloter {activeBatch}</p><p className="text-[10px] text-slate-300 italic">Klik tombol "Tambah Siswa" untuk mengisi data.</p></div>
              </div>
            ) : (
                  ...
                  <div key={student.id} className={`group border rounded-3xl overflow-hidden transition-all ${isInactive ? 'opacity-40 grayscale bg-slate-50' : 'bg-white'} ${isAtSchool ? 'border-emerald-200 bg-emerald-50/10' : 'border-slate-100 shadow-sm'}`}>
                    <div className={`flex items-center gap-4 p-5 ${student.status === StudentStatus.ON_BOARD ? 'bg-indigo-50/50' : ''}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">KLAS: <span className="text-indigo-600">{student.className}</span> • ID: {student.id}</p>
                      </div>
                      <div className="flex gap-2">
                        {!isInactive && !isAtSchool && (
                          <button onClick={() => updateStatus(student.id, student.status === StudentStatus.ON_BOARD ? StudentStatus.AT_HOME : StudentStatus.ON_BOARD)} className={`px-5 py-3 rounded-2xl text-[10px] font-black transition-all shadow-sm ${student.status === StudentStatus.ON_BOARD ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                            {student.status === StudentStatus.ON_BOARD ? 'TURUNKAN' : 'NAIK MOBIL'}
                          </button>
                        )}
                        <button onClick={() => deleteStudent(student.id, student.name)} className="p-3 text-red-300 hover:text-red-600 hover:bg-red-50 rounded-2xl transition-all" title="Hapus Siswa"><Trash2 size={20} /></button>
                      </div>
                    </div>
